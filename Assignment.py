...
def demo():
    ...
    options.set_capability('sessionName', 'Browserstack assignment testcase')
    ...
    try:
        ...
        try:
            cookie = WebDriverWait(driver, 15, 0.5).until(
                EC.visibility_of_element_located((By.XPATH, "//button[@id='didomi-notice-agree-button']")))
            cookie.click()
        except Exception as e:
            print("Cookie not found")

        if lan == 'es-ES':
            print("Website is displayed in Spanish")
        else:
        ...
        try:
            driver.execute_script(
                'browserstack_executor: {"action": "setSessionStatus", "arguments": {"status":"passed", "reason": "Tried to execute the test case"}}')
            driver.quit()
        except:
            pass
